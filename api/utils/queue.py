...
class queue:
    ...
    async def setup(self) -> dict:
        ...
        Returns:
            tuple(dict, dict): current queue dict and all pending tasks
        """
        self.load()  # load in the queue from the queue json

        self.pendingSnipes = {}
        for target in self.targets():
            self.pendingSnipes[target] = await self.add(
                    target,
                    self.queue[target]["offsets"],
                    self.queue[target]["droptime"],
                )
            await asyncio.sleep(1)

        logger.info("Queue initlized successfully")
    ...
    async def add(
        ...
        if (await self.validate(target) is True) or override:
            ...
            # create/return the task
            self.pendingSnipes[target] = asyncio.create_task(
                utils.awaitSnipe(self.config, self, target, offsets, droptime)
            )
            return self.pendingSnipes[target]
    ...
    async def validate(
        ...
        logger.info(f'"{target}" successfully passed validation')
